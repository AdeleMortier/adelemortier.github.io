...
\title{\includegraphics[height=22pt]{./O.png}ddness under Discussion}
...
\begin{frame}{Redundancy}
	\begin{itemize}
		\item A prominent approach to sentences like (\ref{ex:hd}), is based on \textsc{Redundancy}.\footciteia{Grice1975,Horn1984,Meyer2013,Mayr2016,Katzir2014,Kalomoiros2024}\pause
		\item Both of (\ref{ex:hd})'s disjuncts convey the information that \textit{Jo studied in \France}.\pause
...
\begin{frame}{Conceptual advantages of implicit questions}
	\begin{itemize}
		\item Assertive sentences are proposals to \textbf{ update beliefs}, but also suggest ways to \textbf{hierarchically organize such beliefs}, i.e. encapsulate the dynamics of conversations.\pause
		\item Implicit questions (defined in this way) constitute a natural \textbf{extension of Dynamic Semantics}\footcite{Heim1983a} at a more pragmatic or ``inquisitive'' level.
		\item Unlike Inquisitive Semantics,\footnote{ \cite{Mascarenhas2008}; \cite{Ciardelli2009}; \cite{Groenendijk2009}; \cite{Ciardelli2017}; \cite{Ciardelli2018}; \citeauthor{Zhang2024}, to appear}, sentence meanings (propositional content) and the issues they raise (how the content comes about) are not conflated.
	\end{itemize}
\end{frame}
\begin{frame}{Empirical advantages}
	\begin{itemize}
		\item Making pragmatic constraints sensitive to both sentences and their implicit questions, captures cases like (\ref{ex:hd}), (\ref{ex:chd}) and (\ref{ex:hc}), that together challenge standard \textsc{Redundancy}-based approaches to oddness.\pause
...
\begin{frame}{Assertions and questions}
	\begin{itemize}
		\item Assertions typically denotes propositions (\textbf{sets of worlds}).\pause
		\item The set of worlds compatible with the premises of a conversation is called \textbf{Context Set} (\textbf{CS}).\footcite{Stalnaker1978}\pause
		\item Assertions update the CS by \textbf{intersection}.\footciteia{Stalnaker1978,Heim1982, Heim1983a,Heim1983b}\pause
	...
	\begin{figure}[H]
		...
		\begin{tabular}{ccc}
			\begin{tikzpicture}
				...
				\draw [draw=black,pattern=north east lines] (1, 0) rectangle (3, 3);
				\node[] (a) at (.3,2.7) {\textbf{CS}};
				\node[] (a) at (3.2,3.2) {$p$};
			\end{tikzpicture}\pause& \begin{tabular}{c}
			$\stackrel{\cap}{\leadsto}$\\~\\~\\~\\~\\~\\
			\end{tabular} &
			\begin{tikzpicture}
				%\draw [draw=black, fill=gray] (0, 0) rectangle (2, 3);
				\draw [draw=black,pattern=north east lines] (0, 0) rectangle (2, 3);
				\node[] (a) at (.3,2.7) {\textbf{CS}};
			\end{tikzpicture}
		\end{tabular}
...
\begin{frame}{Standard question pragmatics}
	\begin{itemize}
		\item Questions induce a \textbf{partition of the CS}: just group together the worlds of the CS that agree on all alternatives.\footcite{Groenendijk1984}\pause
		\item The resulting groups are called \textbf{cells}: they tell us which distinctions ``matter''.\pause
	...
	\begin{overprint}
		\onslide<3>
			\begin{figure}[H]
				...
				\end{tikzpicture}}\\
				{\textbf{Step 1: }Check how each world deals with the alternatives: \begin{tikzpicture}
						\draw [draw=black,pattern=north west lines] (0, 0) rectangle (.5, .3);
					\end{tikzpicture} defines \textit{Al did the readings} and \begin{tikzpicture}
		...
		\onslide<4->
			\begin{figure}[H]
				...
				\end{tikzpicture}}\\
				{\textbf{Step 2: }Partition the CS by grouping worlds that pattern the same. }\label{fig1:cells}
		\end{figure}
	\end{overprint}
	\begin{itemize}
		\item \onslide<4>We will only consider exhaustive and mutually exclusive alternatives, s.t. question semantics and question pragmatics in fact coincide.
	\end{itemize}\vspace{3mm}
\end{frame}

\begin{frame}{Answering questions}
	\begin{itemize}
		\item Here the cells are \textit{only Ed did the readings}, \textit{only Al}, \textit{Ed an Al}, and \textit{neither}. Those are \textbf{maximal answers}.\pause
	...
	\begin{figure}[H]
		...
		\end{figure}
		\begin{itemize}
			\item \textbf{Questions \textit{encode} maximal answers only}. The non-maximal ones can be \textit{derived} by union. 
		\end{itemize}
\end{frame}
\begin{frame}{Constraints on question-answer pairs: Congruence}
	\begin{itemize}
		\item Question-answer pairs are subject to constraints.\pause
		\item For instance, an answer better be ``congruent'' with the corresponding question. This explains the pattern in (\ref{ex:congruence}).
	\end{itemize}
	\begin{exe}
	...
	\end{exe}
	\begin{exe}
		\ex {\textbf{Who} did the readings?
		}\label{ex:congruence}
		\begin{xlist}
			\ex[] {\textbf{ED} did the readings.}
			\ex[\#] {Ed did the \textbf{READINGS}.}
		\end{xlist}
	\end{exe}
\end{frame}
...
\begin{frame}{A desideratum to guide our framework}
	\begin{itemize}
		\item Overt question answer-pairs match in terms of \textbf{specificity}. This should be a desideratum for implicit questions, too.
	\end{itemize}
	\begin{exe}
		\ex \label{ex:qa-gran}
		\begin{xlist}
...
\begin{frame}{Questions as nested partition}
		...
		\end{figure}
		\begin{itemize}
			\item A ``fine-grained'' question may then contain coarser-grained questions, meaning, a \textit{which city} question \textbf{structurally refines} a \textit{which country} question.
		\end{itemize}\vspace{2mm}
\end{frame}

\begin{frame}{Useful notational variant: questions as Trees}
	\begin{itemize}
		\item  Nested partitions will be represented as \textbf{trees}. The layers of a question-tree have same specificity.\pause
		\item Simple sentences like \textit{Jo studied in \Paris} may then evoke nested ``\textit{wh}'' trees like Fig. \ref{fig:qtree-paris-wh}, or polar trees like Fig. \ref{fig:qtree-paris-polar}.
	\end{itemize}
...
\begin{itemize}\vspace{-4mm}
	\item \textbf{Their deepest layers matches the prejacent's specificity.}\footnote{This recipe already get us the challenging ``compatible'' Hurford cases like (\ref{ex:chd}), (almost) for free!}\vspace{2mm}
\end{itemize}
\end{frame}
...
\begin{frame}{Flagging, and ``negating'' Questions Trees}
	\begin{itemize}
		\item<1-> When a simple assertion evokes an implicit question tree, leaves entailing the assertion get flagged; \textbf{flags track ``at-issue'' meaning, and are compositionally derived}.
		\item<2-> Negating an assertion \textbf{flips the flags} on this assertion's trees. Flag-flipping is a layerwise \textbf{complement set} operation, which \textbf{does not affect the specificity of the underlying question-tree}.
	\end{itemize}
	\begin{overprint}
	\onslide<1>
		\begin{figure}[H]
...
\begin{frame}{Disjoining Questions Trees}
	\begin{itemize}
		\item Disjunction \textbf{fuses} the trees evoked by the disjuncts, retaining only unions that are well-formed nested partitions.\pause
...
\begin{frame}{Interim summary: expressivity of question-trees}
	\begin{itemize}
		\item Questions were modeled as \textbf{nested partitions}, represented as trees. Even if they look bulkier, they are just the inductive closure of an existing, incontroversial object: partitions of the CS.\pause
	...
	\end{itemize}
\end{frame}
\begin{frame}{Interim summary: transparency of question-trees}
	\begin{itemize}
		\item Disjunctions and conditionals can evoke different tree structures, \textit{independently of their assigned semantics}:\pause
		\begin{itemize}
			\item Disjunctive trees are formed with $\cup$, capturing the idea that \textbf{disjuncts answer the same global question}.\footcite{Simons2001,Zhang2022,Westera2020}\pause
...
\begin{frame}{Q-Non-Redundancy}
	\begin{itemize}
		\item Recall \textsc{Redundancy} usually arises when a sentence has the same logical content as one of its simplifications.\pause
		\item We generalize this to sentence-tree pairs: \textsc{Q-Redundancy} arises for a sentence-tree pair, if \textbf{a simplification  of the sentence, yields an ``equivalent'' tree}.\pause
		\item Tree equivalence is understood as \textbf{structural identity} plus \textbf{equality of minimal paths from the root to all }\faFlagCheckered.
	\end{itemize}
\end{frame}

\begin{frame}{Capturing HDs}
	\begin{itemize}
		\item The HD \textit{\Paris{} or \France}, is then odd because its only implicit tree, is equivalent to a tree evoked by the \Paris-disjunct.\pause
...
\begin{frame}{Describing the contrast in HCs}
	...
	\end{exe}
	\begin{itemize}\pause
		\item Descriptively, (\ref{ex:hc-ws}) and \#(\ref{ex:hc-sw}) only differ in:\pause
		\begin{enumerate}[(i)]
			\item where \textbf{overt negation} is: having it in the antecedent triggers \#.\pause
			\item how antecedents and consequents are \textbf{ordered in terms of specificity}: \stronger{fine}-to-\weaker{coarse} progressions are \#.
		\end{enumerate}\pause
		\item To capture HCs \citet{Kalomoiros2024}'s \textsc{Super Redundancy} constraint exploited (i); we choose to exploit (ii).
		\item This will make way for a more intuitive account, recycling a familiar concept (\textsc{Relevance}) at the subsentential level.
	\end{itemize}
\end{frame}
...
\begin{frame}{Capturing odd HCs: case 1}
	...
	\begin{minipage}{.55\linewidth}
		\begin{itemize}
			\item A country-level tree gets plugged into a \textit{not \Paris}-leaf.\pause
			\item The leaves that remains are all countries, but \France is intersected with \textit{not \Paris}.\pause
			\item This violates \textsc{Incremental Q-Relevance}, because none of the original leaves is \textbf{fully excluded}.\pause
...
\begin{frame}{Capturing odd HCs: case 2}
	...
	\begin{minipage}{.55\linewidth}
		\begin{itemize}
			\item A country-level tree gets plugged into a \textit{not \Paris}-leaf.\pause
			\item The leaves that remains are all smaller than countries -- in fact they get shrunk into city-leaves.\pause
			\item This violates \textsc{Incremental Q-Relevance}, because no original leaf is \textbf{fully retained}.\pause
...
\begin{frame}{Additional remarks about \textsc{Incremental Q-Relevance}}
	\begin{itemize}
		\item \textsc{Incremental Q-Relevance} imposes that some, but not all distinctions introduced by the question being restricted, are retained; domain restrictions must be \textbf{faithful to the specificity} of the original question, but also \textbf{relevantly informative}.\pause
		\item Antecedents (i.e. restrictors) that are too specific will not allow the leaves of the consequent to properly ``fit'' in the domain(s) they define.\pause
		\item The ``incremental'' character of the constraint \textbf{piggybacks on the asymmetric definition assigned to conditional question-trees}: the roles of the antecedent and consequent are asymmetric, and so are violations of \textsc{Incremental Q-Relevance}.
	\end{itemize}
\end{frame}

\begin{frame}{Further teasing apart specificity vs. overt negation}
	\begin{itemize}
		\item \textsc{Incremental Q-Relevance} ends up capturing subtle asymmetries in ``compatible'' variants of HCs, whose oddness seems\textbf{ more specificity-sensitive (in a weaker sense) than negation-sensitive}.
	\end{itemize}
	\begin{exe}
		\ex\label{ex6:chc}
		\begin{xlist}
	...
	\end{exe} \pause
	\begin{itemize}
		\item This further supports the current view, against \citet{Kalomoiros2024}'s earlier view of HCs.
	\end{itemize}
\end{frame}
